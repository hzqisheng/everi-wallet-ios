...
- (NSUInteger)daysLater {
    return [NSDate daysLater:self];
}

+ (NSUInteger)daysLater:(NSDate *)date {
    NSCalendar *calendar = [NSCalendar currentCalendar];
...
+ (NSDate *)yearMonthToDate:(NSInteger)year month:(NSInteger)month {
    ...
    NSString *dataStr = [NSString stringWithFormat:@"%ld-%@",(long)year,monthStr];
    
    return [NSDate dateWithString:dataStr format:@"yyyy-MM"];
}
...
+ (NSUInteger)daysInMonth:(NSUInteger)month year:(NSUInteger)year {
    ...
    NSDate *date = [NSDate dateWithString:dataStr format:@"yyyy-MM"];
    
    return [NSDate daysInMonth:date month:month];
}
...
//    NSDate *date = [self dateWithString:dateString format:[self ymdHmsFormat]];
    ...
    double millisecond = [dateString doubleValue];
