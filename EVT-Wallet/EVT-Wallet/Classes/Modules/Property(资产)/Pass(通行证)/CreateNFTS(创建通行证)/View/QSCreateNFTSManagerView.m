...
@implementation QSCreateNFTSManagerView
...
- (void)didClickEditButton {
    self.onlyButton.selected = NO;
    self.manualButton.selected = YES;
    if (self.createNFTSManagerClickEditButtonBlock) {
        self.createNFTSManagerClickEditButtonBlock();
    }
}
...
- (UILabel *)titleLabel {
    if (!_titleLabel) {
        ...
        _titleLabel.text = QSLocalizedString(@"qs_issue_issue_moreSetting_manage");
...
- (UILabel *)onlyLabel {
    if (!_onlyLabel) {
        ...
        _onlyLabel.text = QSLocalizedString(@"qs_pass_createNFTS_manager_content");
