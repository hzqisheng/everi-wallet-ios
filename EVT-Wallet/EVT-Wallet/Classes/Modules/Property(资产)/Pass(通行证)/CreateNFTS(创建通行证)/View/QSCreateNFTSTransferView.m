...
@implementation QSCreateNFTSTransferView
...
- (void)didClickEditButton {
    self.onlyButton.selected = NO;
    self.manualButton.selected = YES;
    if (self.createNFTSTransferViewClickEditButtonBlock) {
        self.createNFTSTransferViewClickEditButtonBlock();
    }
}
...
- (UILabel *)titleLabel {
    if (!_titleLabel) {
        ...
        _titleLabel.text = QSLocalizedString(@"qs_issue_issue_permissions_transfer");
...
- (UILabel *)onlyLabel {
    if (!_onlyLabel) {
        ...
        _onlyLabel.text = QSLocalizedString(@"qs_pass_createNFTS_transfer_content");
