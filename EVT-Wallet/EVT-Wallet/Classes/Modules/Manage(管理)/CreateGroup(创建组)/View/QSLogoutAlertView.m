...
@interface QSLogoutAlertView ()<UIGestureRecognizerDelegate>
...
@implementation QSLogoutAlertView

+ (void)showLogoutAlertViewAndSubmitBlock:(void (^)(void))block {
    QSLogoutAlertView *view = [[QSLogoutAlertView alloc] initWithFrame:kScreenBounds];
    view.logoutAlertViewSubmitBlock = block;
...
- (void)submitButtonClicked {
    NSMutableArray *arr = [NSMutableArray arrayWithArray:[[QSWalletHelper sharedHelper] getWalletArray]];
    if (arr.count > 0) {
        QSCreateEvt *evt = arr[0];
        if (![self.privateTextField.text isEqualToString:evt.password]) {
            [QSAppKeyWindow showAutoHideHudWithText:QSLocalizedString(@"qs_alert_content_password")];
            return;
        }
        if (self.logoutAlertViewSubmitBlock) {
            self.logoutAlertViewSubmitBlock();
